  ...
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        // Le rouge accent devient une variante nommée plutôt qu'un `bg-accent`
        // posé à la main : la règle « un seul rouge par écran, CTA et prix
        // uniquement » redevient vérifiable au lieu d'être une convention orale.
        cta: "bg-accent text-accent-foreground font-semibold shadow-sm hover:bg-accent/90 active:scale-[0.99]",
      },
      // Cibles tactiles : 44 px minimum (WCAG 2.5.5) sur tout ce qu'une
      // cliente touche. Les anciennes valeurs (h-8 = 32 px, lg = 36 px)
      // étaient toutes sous la norme, ce que trahissaient les 56 surcharges
      // manuelles de hauteur éparpillées dans les composants.
      // Les variantes `xs` / `icon-xs` restent compactes : densité back-office
      // au clavier et à la souris uniquement, jamais sur le parcours client.
      size: {
        default:
